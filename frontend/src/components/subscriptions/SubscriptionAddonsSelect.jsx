...
export default function SubscriptionAddonsSelect({ value = [], onChange, onCatalogItemsChange }) {
  ...
  useEffect(() => {
    let ignore = false;
    api.get('catalog-items/', { params: { category: 'addon', is_active: 'true' } })
      .then(({ data }) => {
        if (ignore) return;
    ...
    <div className="grid gap-2 md:col-span-2">
      ...
      {!items.length ? (
        <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-500">
          Дополнительные услуги не добавлены. Добавьте их в Настройки → Доп. услуги.
        </div>
      ) : (
